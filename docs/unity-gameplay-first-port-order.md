...
| 17 | Graphics Parity | IN PROGRESS | 2026-03-29 | | Sub-phase 17.1-17.5 DONE: SceneTileRenderer.cs (17 scene types, procedural tile textures matching JS tileRenderer.js, deterministic variation, decorations, scene BG colors), TextureCanvas blend methods (BlendFillRect/StrokeRect/Circle/Line/Ellipse), CharacterRenderer enhancements (CHAR_SCALE, walk bob, armor tier tint, status tint), MobRenderer healer float, HitEffectRenderer.cs (damage numbers, kill rewards, heal text, status ticks — pooled), DamageSystem wired to spawn hit effects, GraphicsParityTest.cs (20 tests). YSortRenderer already matches JS. REMAINING: 172 entity renderers, full 73 hit effect types, bullet visual styles, Mafia FOV, security cameras, scene overlays, Among Us crewmate renderer |
