...
| 16 | Secondary Systems | DONE | 2026-03-29 | 2026-03-29 | All 10 sub-phases complete (16.1-16.10 + 16.5 Save/Load) |
| 16.6 | Hide & Seek | DONE | 2026-03-29 | 2026-03-29 | HideSeekData.cs (timers, spawns from level data match JS, FOV, bot tuning), HideSeekSystem.cs (5-phase FSM, role assignment, freeze logic, tag detection; bot waypoint/hide algorithms stubbed), SocialModesParityTest.cs (8 tests: timers, phases, freeze, tag range) |
| 16.7 | Mafia | DONE | 2026-03-29 | 2026-03-29 | MafiaData.cs (9 participants, kill/discussion/voting/ejection timers, 10 colors, 3 sabotage types, 7 roles with subrole timing, Skeld room centers, lobby settings all match JS), MafiaSystem.cs (9-phase FSM, role assignment, kill, vote, sabotage, win conditions; bot AI stubbed), SocialModesParityTest.cs (17 tests: counts, timers, colors, sabotage, roles, settings) |
| 16.8 | Casino | DONE | 2026-03-29 | 2026-03-29 | CasinoData.cs (BET_MIN/MAX, HOUSE_EDGE=0.05, all 10 game configs match JS), CasinoSystem.cs (all 10 games: blackjack split/insurance/dealer, roulette, coinflip streaks, cases weighted, mines grid, dice, RPS bo1/3/5, baccarat 3rd card, slots jackpot, keno 3-risk), CasinoSparParityTest.cs (14 tests: bet limits, payouts, multiplier formulas) |
| 16.9 | Spar | DONE | 2026-03-29 | 2026-03-29 | SparData.cs (HP_BASELINE=100, COUNTDOWN=180, MAX_MATCH=3600, 8 rooms, CT-X formulas: FreezePenalty/FireRate/Spread/Reload, 3 duel styles, 3 meta builds, learning profile v8 all match JS), SparSystem.cs (match lifecycle, team creation, CT-X allocation, shooting, win resolution; bot AI stubbed for Unity remake), CasinoSparParityTest.cs (11 tests: HP, timers, rooms, CT-X formulas, meta builds, win resolution) |
| 16.5 | Party + Bot AI | DONE | 2026-03-29 | 2026-03-29 | PartyData.cs FIXED: gun damage 28 (was 20), mag 35 (was 30), fireRate 5 (was 10), added DEATH_ANIM_FRAMES=40/RESPAWN_COUNTDOWN=180/BOT_SHOOT_CD=10/BOT_MELEE_CD=20. PartySystem.cs FIXED: deathTimer=0.667s (was 1.5s), respawnTimer uses constant. BotAI.cs: 5-state FSM, smart targeting, DPS melee/gun, separation. PartyBotParityTest.cs (19 tests: config, HP scaling, ranges, mob scaling, revive, potions, gun/melee stats, death/respawn timers, FSM, combat style, spread) |
...
| 17 | Graphics Parity | IN PROGRESS | 2026-03-29 | | **Slice 1** DONE: SceneTileRenderer.cs (17 scene types), TextureCanvas blend methods, CharacterRenderer enhancements, MobRenderer healer float, GraphicsParityTest.cs (20 tests). **Slice 2** DONE: HitEffectRenderer.cs full rewrite — all 73 JS HIT_EFFECT_RENDERERS types ported (data-driven config with 6 visual templates: Burst, Ring, Slash, Lightning, Particles, FloatingText), pooled SpriteRenderer+TextMesh+LineRenderer primitives, procedural circle/ring/slash sprites. DamageSystem isCrit routing, MobController burn_tick effects, thorns effect spawn, SpawnEffect generic API + 7 convenience methods. HitEffect.cs expanded (angle, gold, text, color, endPos, originPos, healAmt). **Slice 3** DONE: BulletSystem.cs visual upgrade — Graal-style 30x8px bullet sprites, procedural arrow/boulder/circle sprites, 13 projectile styles (neon_bolt, tracer, golden, saw_blade, poison arrow, boulder, generic + custom bulletColor), trail glow children, saw blade spin, boulder tumble. Bullet struct expanded with projectileStyle/isArrow/isBoulder/bulletColor fields. **Slice 4** DONE: HideSeekFOV.cs (circular FOV mask seek phase + map overview hide phase, per-pixel alpha mask), MafiaFOV.cs (DDA wall-aware raycasting, 120 rays, blur-simulated polygon cutout, lights sabotage dimming, O2 fog overlay, IsWorldPointVisible query), SecurityCameraOverlay.cs (4 RenderTexture feed cameras, 2×2 grid UI, scan lines, REC blink, green tint, timestamps), StatusOverlays.cs (blind vignette radial gradient with flash/dark modes, poison glow world-space sprite, ghost tint+label), CrewmateRenderer.cs (Among Us crewmate with procedural parts: body/backpack/visor/legs/shoes, walk anim, ghost alpha, direction flip, EnableOn/DisableOn API), MinimapUI.cs upgraded (MINIMAP_LABELS room labels for Skeld 14 rooms burned into texture + text overlays, task/sabotage dot markers, party member dots, tracker target dot, dynamic dot pooling). REMAINING: 172 entity renderers, mob-owned world objects (turrets, drones, mines, etc) |
