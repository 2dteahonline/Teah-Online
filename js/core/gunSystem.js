...
function getMuzzlePos(aimDir) {
  ...
  if (aimDir === 0) { // down — default: gun on screen-right (bodyR). Mirror: screen-left (bodyL)
    ...
    return isRight
      ? { x: bodyR + 1, y: y + 28 - 49 }
      : { x: bodyL - 1, y: y + 28 - 49 };
  } else if (aimDir === 2) { // left — mirrored side shifts gun to opposite Y (same gap as up/down X)
    return isRight
      ? { x: bodyL + 2 - 49, y: armY - 18 }
      : { x: bodyL + 2 - 49, y: armY + 18 };
  } else { // right — mirrored side shifts gun to opposite Y
    return isRight
      ? { x: bodyR + 9 + 49, y: armY + 18 }
      : { x: bodyR + 9 + 49, y: armY - 18 };
  }
}
...
function shoot() {
  ...
  if (pelletCount > 0) {
    ...
    for (let s = 0; s < stackCount; s++) {
      ...
      // Spar: tag player bullets with team + damage + context for learning
      if (typeof SparState !== 'undefined' && SparState.phase === 'fighting') {
        ...
        bulletObj._sparDir = player.dir;
        bulletObj.startX = bulletObj.x;
        bulletObj.startY = bulletObj.y;
      }
...
const _mgSliders = {
  ...
  rof: {
    ...
    min: 0, max: 100, step: 5,
    // Piecewise: below 50 +0.075s/10, above 50 -0.025s/10. 50=0.31s, clamped min 3.525
    get: () => (typeof _ctxRof !== 'undefined') ? _ctxRof : 50,
    set: (v) => {
      _ctxRof = v;
      let frames;
      if (v <= 50) {
        frames = 10.275 - v * 0.1125; // 0→0.685s, 50→0.31s
      } else {
        frames = Math.max(3.525, 4.650 - (v - 50) * 0.0375); // 50→0.31s, 80+→0.235s
      }
...
// Sync CT_X_GUN with default slider values once all scripts are loaded
