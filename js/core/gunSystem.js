...
const _mgSliders = {
  freeze: {
    ...
    // Linear: 0→0.50, 50→0.25, 100→0.00. 0.05 per 10. Duration flat 15f.
    get: () => (typeof _ctxFreeze !== 'undefined') ? _ctxFreeze : 50,
    set: (v) => {
      _ctxFreeze = v;
      const penalty = 0.50 - v * 0.005; // 0.05 per step of 10, no cap
  ...
  rof: {
    ...
    min: 0, max: 100, step: 5,
    // Linear: 0→5.39f, 100→1.23f (50=0.22s per shot)
    get: () => (typeof _ctxRof !== 'undefined') ? _ctxRof : 50,
    set: (v) => {
      _ctxRof = v;
      const frames = 5.39 - (v / 100) * 4.16; // 0→5.39, 50→3.31, 100→1.23
