    ...
    if (m.boulderRate > 0) {
      ...
      if (m.boulderTimer > 0) {
        ...
        if (stompDist < stompRange) {
          ...
          if (stompDist > 0) {
            applyKnockback((stompDx / stompDist) * 12.6, (stompDy / stompDist) * 12.6);
          }
        }
  ...
  mini_golem: (m, ctx) => {
    ...
    if (m.boulderTimer > 0) {
      ...
      if (stompDist < stompRange) {
        ...
        if (stompDist > 0) {
          applyKnockback((stompDx / stompDist) * 7, (stompDy / stompDist) * 7);
        }
      }
      ...
      if (m._poundTelegraph <= 0) {
        ...
        if (typeof AttackShapes !== 'undefined') {
          if (AttackShapes.hitsPlayer(pcx, pcy, 96, player)) {
            ...
            applyKnockback((kDx / kDist) * 8.4, (kDy / kDist) * 8.4);
            // Slow
      ...
      if (m._pileDriverTelegraph <= 0) {
        ...
        if (typeof AttackShapes !== 'undefined') {
          if (AttackShapes.hitsPlayer(pcx, pcy, 96, player)) {
            ...
            const kDx = player.x - pcx, kDy = player.y - pcy;
            const kDist = Math.sqrt(kDx * kDx + kDy * kDy) || 1;
            applyKnockback((kDx / kDist) * 9.8, (kDy / kDist) * 9.8);
          }
        }
        hitEffects.push({ x: pcx, y: pcy, life: 20, type: "stomp" });
      }
