...
function updateMobs() {
  ...
  for (const m of mobs) {
    if (m.hp <= 0) continue;
    // (Hide & Seek bot is a standalone entity, not in mobs[])
    ...
    if (m._despawnTimer !== undefined && m._despawnTimer > 0) {
      m._despawnTimer--;
      if (m._despawnTimer <= 0) { m.hp = 0; continue; }
    }
        ...
        if (m._activeAbility) {
          ...
          if (handler) {
            ...
            const stillActive = m._laserTelegraph > 0 || m._tommyFiring ||
              ...
              m._takeoverTelegraph > 0 || m._takeoverPulling ||
              // Floor 3
